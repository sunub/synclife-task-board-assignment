...
- Move: status 변경, client sequence, stale response suppression, 409 rebase retry, previous task rollback
...
409 conflict response가 발생하면 mutation 종류에 따라 다음 정책을 따른다.

Move:

1. 오래된 move 요청의 409는 client sequence 비교로 무시한다.
2. 최신 move 요청의 409에 `current` task가 있으면 이를 cache에 반영해 서버 기준점을 맞춘다.
3. `current.status`가 사용자의 마지막 target status와 다르고 아직 rebase retry를 하지 않았다면, 같은 target status를 `current.version`으로 한 번 더 PATCH한다.
4. rebase retry가 성공하면 반환된 task를 cache에 확정한다.
5. rebase retry까지 409가 발생하거나 서버 최신 상태가 이미 target status와 같으면 서버 최신 상태를 반영하고 conflict message를 표시한다.
...
Edit은 사용자의 draft를 새 server version 기준으로 자동 재시도하지 않는다. 텍스트/우선순위/설명 변경은 서버 최신 값과 사용자 draft 사이의 merge 정책이 필요하므로 move의 status-only rebase retry와 분리한다.
