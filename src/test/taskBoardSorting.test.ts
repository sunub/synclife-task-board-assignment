...
import { normalizeTasks, type TaskBoardModel } from "../lib/tasks";
...
describe("작업 보드 정렬 기준", () => {
  it("제목 정렬 기준을 선택하면 같은 컬럼의 작업을 가나다순으로 정렬한다", () => {
    const lastTitleTask = makeTask("a-last-title", { title: "다음 배포 준비" });
    const firstTitleTask = makeTask("b-first-title", { title: "가계약서 검토" });
    const middleTitleTask = makeTask("c-middle-title", { title: "나중 결제 오류 수정" });
  ...
  it("우선순위 정렬 기준을 선택하면 높은 우선순위 작업을 먼저 배치한다", () => {
    ...
    const highPriorityTask = makeTask("b-high-priority", { priority: "high" });
